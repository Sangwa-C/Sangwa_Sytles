...
public class NewAccountActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private boolean isValidPassword(String password, String confirmPassword) {
        if(password.length() < 6){
            mPasswordEditText.setError("Please create a password containing 6 characters minimum");
            return false;
        } else if (!password.equals(confirmPassword)){
            mPasswordEditText.setError("Passwords doesn't match");
            return false;
        }
        return true;
    }
    ...
    private boolean isValidEmail(String email) {
        boolean isGreatEmail = (email != null && Patterns.EMAIL_ADDRESS.matcher(email).matches());
        if(!isGreatEmail){
